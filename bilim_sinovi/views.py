...
from .models import Question, TestConfig, TestResult
...
def _get_test_questions(test_config):
    subject_ids = test_config.get_effective_subject_ids()
    if not subject_ids:
        return []

    queryset = Question.objects.filter(subject_id__in=subject_ids).select_related('subject')
...
class TestListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        now = timezone.now()
        return TestConfig.objects.filter(
            is_active=True,
            start_time__lte=now,
            end_time__gte=now,
        ).order_by('end_time')
...
class TestManageListView(LoginRequiredMixin, TestOrganizerRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return TestConfig.objects.select_related('subject').prefetch_related('question_sets').order_by('-created_at')
